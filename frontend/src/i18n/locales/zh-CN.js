export default {
  login: {
    title: 'MebTTY',
    signIn: '登录您的账户',
    signUp: '创建新账户',
    username: '用户名',
    password: '密码',
    usernamePlaceholder: '请输入用户名',
    passwordPlaceholder: '请输入密码',
    signInBtn: '登录',
    signUpBtn: '创建账户',
    pleaseWait: '请稍候...',
    noAccount: '还没有账户？',
    hasAccount: '已有账户？',
    register: '注册',
    enterBoth: '请输入用户名和密码',
    authFailed: '认证失败'
  },
  home: {
    welcome: '欢迎回来，',
    subtitle: '在一个地方管理您的终端会话。',
    totalSessions: '总会话数',
    running: '运行中',
    openTabs: '打开标签',
    newTerminal: '新建终端',
    openTerminal: '打开终端',
    runningSessions: '运行中的会话',
    recentSessions: '最近的会话',
    logout: '退出登录',
    newSession: '新建终端会话',
    shell: 'Shell',
    localUsername: '本地用户名',
    localUsernamePlaceholder: '本地系统用户',
    localPassword: '本地用户密码',
    localPasswordPlaceholder: '该本地用户的密码',
    localLoginFailed: '登录本地用户失败',
    cancel: '取消',
    create: '创建',
    creating: '创建中...',
    settings: '设置',
    deleteSession: '删除',
    confirmDelete: '确定要删除此会话吗？'
  },
  terminal: {
    noActive: '没有活动的终端',
    noActiveDesc: '创建一个新的终端会话以开始',
    newTerminal: '新建终端',
    home: '主页',
    logout: '退出登录',
    welcome: '欢迎',
    welcomeSubtitle: '创建一个新终端来开始吧。',
    quickStart: '快速上手',
    tipNew: '点击标签栏的 + 按钮或下方按钮创建终端',
    tipSwitch: '点击标签页切换终端',
    tipRename: '双击标签页可重命名',
    tipClose: '关闭标签页会自动删除对应的会话',
    tipFiles: '点击文件夹图标浏览文件',
    tipSettings: '点击头像进入设置'
  },
  toolbar: {
    new: '新建',
    search: '搜索',
    fileBrowser: '文件浏览器',
    zenMode: '禅模式',
    uploadFile: '上传文件',
    switchToLight: '切换到亮色模式',
    switchToDark: '切换到暗色模式',
    themeSystem: '跟随系统主题',
    settings: '设置'
  },
  statusBar: {
    connected: '已连接',
    disconnected: '已断开'
  },
  tabs: {
    close: '关闭',
    newTerminal: '新建终端',
    rename: '重命名',
    icon: '图标',
    changeIcon: '更改图标...',
    searchIcon: '搜索图标',
    noIcons: '没有找到图标'
  },
  fileBrowser: {
    explorer: '资源管理器',
    upload: '上传到所选目录',
    newFolder: '在所选目录中新建文件夹',
    refresh: '刷新',
    close: '关闭',
    loading: '加载中...',
    emptyDir: '空目录',
    noProvider: '没有可用的文件提供者',
    download: '下载',
    newFolderHere: '在此新建文件夹',
    rename: '重命名',
    delete: '删除',
    copyPath: '复制路径',
    newFolderTitle: '新建文件夹',
    folderNamePlaceholder: '文件夹名称',
    create: '创建',
    cancel: '取消',
    renameTitle: '重命名',
    deleteFolder: '删除文件夹',
    deleteFile: '删除文件',
    deleteConfirm: '确定要删除 {name} 吗？',
    failedCreateFolder: '创建文件夹失败',
    failedRename: '重命名失败',
    failedDelete: '删除失败',
    showHidden: '显示隐藏文件'
  },
  fileEditor: {
    loading: '加载中...',
    saving: '保存中...',
    saved: '已保存',
    unsaved: '有未保存修改',
    ready: '就绪',
    readOnly: '只读',
    save: '保存',
    close: '关闭',
    openFailed: '打开文件失败',
    saveFailed: '保存文件失败',
    unknownType: '未知文件类型',
    discardConfirm: '放弃未保存的修改吗？'
  },
  settings: {
    title: '设置',
    back: '返回',
    categories: '设置分类',
    sectionAppearance: '外观',
    sectionTerminal: '终端',
    sectionFiles: '文件',
    sectionStatus: '状态栏',
    sectionPlugins: '插件',
    sectionAccount: '账户',
    language: '语言',
    languageDesc: '选择应用程序的显示语言',
    languageAuto: '自动（跟随浏览器）',
    colorMode: '颜色模式',
    colorModeDesc: '选择应用程序的外观',
    pluginTheme: '插件主题',
    pluginThemeDesc: '应用已启用插件提供的主题',
    pluginThemeCustom: '自定义颜色',
    system: '跟随系统',
    dark: '深色',
    light: '浅色',
    accentColor: '主题色',
    accentColorDesc: '自定义界面的主色调',
    customTheme: '自定义主题',
    customThemeDesc: '使用自己的配色覆盖界面的核心颜色',
    customThemeColors: '主题颜色',
    customThemeColorsDesc: '分别编辑深色和浅色模式的基础颜色',
    resetCustomTheme: '重置主题颜色',
    themeBg: '背景',
    themeBgDeep: '深层背景',
    themeSurface: '表面',
    themeSurfaceHover: '表面悬停',
    themeOverlay: '覆盖层',
    themeText: '文本',
    themeSubtext: '次级文本',
    themeBorder: '边框',
    themeAccent: '强调色',
    tabTitleFormat: '标签页标题格式',
    tabTitleFormatDesc: '自定义终端标签页标题的显示方式',
    tabTitlePreview: '预览',
    avatar: '头像',
    avatarDesc: '上传个人头像',
    uploadAvatar: '上传',
    changeAvatar: '更换头像',
    changePassword: '修改密码',
    changePasswordDesc: '更新您的账户密码',
    currentPassword: '当前密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    passwordChanged: '密码修改成功',
    passwordMismatch: '两次输入的密码不一致',
    passwordTooShort: '密码至少需要4个字符',
    sidebarPosition: '文件侧边栏位置',
    sidebarPositionDesc: '选择文件浏览器显示在哪一侧',
    sidebarMode: '侧边栏显示模式',
    sidebarModeDesc: '选择侧边栏占据布局空间或悬浮在终端上方',
    sidebarDocked: '停靠',
    sidebarOverlay: '悬浮',
    fileIconPack: '文件图标包',
    fileIconPackDesc: '选择已启用插件提供的图标包',
    left: '左侧',
    right: '右侧',
    statusBar: '状态栏',
    statusBarDesc: '显示或隐藏底部状态栏',
    fileAutoSave: '文件自动保存',
    fileAutoSaveDesc: '编辑文本文件后自动保存修改',
    fileLineNumbers: '文件行号',
    fileLineNumbersDesc: '在文本文件预览和编辑器中显示行号',
    statusBarItems: '状态栏显示项',
    statusBarItemsDesc: '选择状态栏中显示哪些信息',
    statusItemConnection: '连接状态',
    statusItemShell: 'Shell 类型',
    statusItemStatus: '进程状态',
    statusPositionLeft: '左侧',
    statusPositionRight: '右侧',
    dragToReorder: '拖拽排序',
    sessionTimeout: '会话自动删除',
    sessionTimeoutDesc: '在指定时间后自动删除已停止的会话',
    timeoutDisabled: '关闭',
    hours: '小时',
    pluginInstall: '安装插件',
    pluginInstallDesc: '上传 .mtpx 插件包。第三方插件安装后默认不会启用。',
    pluginChoosePackage: '选择插件包',
    pluginInstalling: '安装中...',
    pluginRefresh: '刷新',
    pluginRefreshing: '刷新中...',
    installedPlugins: '已安装插件',
    installedPluginsDesc: '管理内置插件和已安装的第三方插件',
    pluginSettings: '插件设置',
    pluginSettingsDesc: '已启用插件贡献的设置项',
    pluginAdminRequired: '安装或管理插件需要管理员权限。',
    pluginLoading: '正在加载插件...',
    noPlugins: '没有插件',
    pluginBuiltin: '内置',
    pluginThirdParty: '第三方',
    pluginEnable: '启用',
    pluginDisable: '禁用',
    pluginDelete: '删除',
    pluginDeleteConfirm: '确定要删除插件 {name} 吗？',
    pluginInstalled: '插件已安装',
    pluginEnabled: '插件已启用',
    pluginDisabled: '插件已禁用',
    pluginDeleted: '插件已删除',
    pluginLoadFailed: '加载插件失败',
    pluginInstallFailed: '安装插件失败',
    pluginActionFailed: '插件操作失败',
    pluginStatusInstalled: '已安装',
    pluginStatusEnabled: '已启用',
    pluginStatusDisabled: '已禁用',
    pluginStatusError: '错误',
    pluginTypeTheme: '主题',
    pluginTypeIconPack: '图标包',
    pluginTypeFileProvider: '文件提供者',
    pluginTypePanel: '面板',
    pluginTypeIntegration: '集成',
    pluginPermission: {
      ui: {
        panel: '面板',
        toolbar: '工具栏',
        settings: '设置'
      },
      theme: {
        provide: '主题'
      },
      icons: {
        provide: '图标'
      },
      filesystem: {
        read: '读文件',
        write: '写文件',
        delete: '删文件',
        upload: '上传',
        download: '下载'
      },
      network: {
        client: '网络'
      },
      terminal: {
        read: '读终端',
        write: '写终端'
      }
    },
    saved: '已保存',
    delete: '删除',
    deleteConfirm: '确定要删除此会话吗？'
  },
  common: {
    session: '会话'
  }
}
