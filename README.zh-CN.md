...
MebTTY 还提供一套一等公民的**插件系统**。文件浏览器、默认主题和图标包都以插件形式注册；管理员可以安装 `.mtpx` 插件包，为终端添加面板、工具栏按钮、设置项、主题、图标包和文件 provider。
...
- **触屏扩展键盘栏** — 为 iPad 和移动终端提供 Esc、Tab、修饰键、方向键及翻页键
...
- **右键菜单** — 右键点击快速执行文件操作
- **本地用户默认目录** — 首次打开跟随活动终端 `cwd`；否则从最近登录过的本地终端用户家目录开始
- **可配置根目录** — 使用 `MEBTTY_BROWSE_ROOT` 覆盖文件浏览器根目录
- **路径安全检查** — 相对路径从解析后的浏览根目录开始；绝对路径会被规范化并依赖系统权限控制
...
- **完整自定义主题** — 深色和浅色配色 token 均可单独自定义
- **插件主题与图标** — 已启用插件可以贡献可选择的主题和文件图标包
- **可配置状态栏** — 显示/隐藏、拖拽排序项目（shell、进程状态、连接状态），左右定位
- **侧边栏位置** — 选择文件浏览器在左侧或右侧

### 插件

- **可安装 `.mtpx` 包** — 可从设置页或插件 API 上传安装插件包
- **内置插件基础** — 文件浏览器、默认主题和 Catppuccin 图标均为随 MebTTY 更新的内置插件
- **运行时贡献能力** — 插件可以添加终端面板、工具栏按钮、设置区块、主题、图标包和文件 provider
- **管理员管理** — 安装、启用、禁用和删除插件需要管理员账户
- **包校验** — 安装前校验插件 ID、schema、权限、安全 zip 路径、文件数量和解压后大小
- **前端插件 API** — 前端插件可获得 Vue helper 和带认证的 `ctx.api` 客户端
...
- **首个用户自动成为管理员** — 第一个注册的 MebTTY 用户会成为管理员；旧数据库没有管理员时会提升最早创建的用户
- **终端本地用户登录** — 创建终端时可以通过 PAM 认证本地系统用户，并以该用户身份运行 PTY
- **用户头像** — 上传和显示头像（PNG、JPEG、WebP、GIF）
- **审计日志** — 跟踪所有用户操作和执行的命令，带风险等级
- **管理员控制** — 仅管理员可查看审计事件列表和管理插件；按用户隔离访问
- **密码管理** — 通过当前密码验证后修改密码
...
| 插件   | `.mtpx` zip 包，基于 manifest 的前端运行时      |
...
./deploy.sh --local-test # 启动前后端热重载测试服务
...
文件浏览器不会把 systemd 服务账户的家目录作为正常起始目录。从活动终端打开时，它会从该终端当前工作目录开始；如果还没有活动终端路径，则回退到当前 MebTTY 用户最近登录过的本地终端用户家目录。只有需要强制所有文件浏览请求使用固定根目录时，才设置 `MEBTTY_BROWSE_ROOT`。
...
| `/var/lib/mebtty/plugins`         | 第三方插件安装目录           |
...
| `MEBTTY_BROWSE_ROOT`                 | 本地终端用户家目录                 | 可选的文件浏览器固定根目录           |
| `MEBTTY_AUTH_RATE_LIMIT_ATTEMPTS`    | `5`                                | 错误认证达到多少次后锁定；设为 `0` 表示禁用 |
| `MEBTTY_AUTH_RATE_LIMIT_WINDOW_SECONDS` | `300`                            | 统计错误认证次数的时间窗口，单位秒   |
| `MEBTTY_AUTH_RATE_LIMIT_LOCKOUT_SECONDS` | `300`                           | 错误认证过多后的锁定时间，单位秒     |
...
| `MEBTTY_REGISTRATION_ENABLED`        | `true`                             | 是否允许创建新的 MebTTY Web 账户     |
...
| `MEBTTY_PLUGIN_DIR`                  | `./plugins`                        | 上传插件的安装目录                   |
| `MEBTTY_PLUGIN_MAX_SIZE`             | `20971520`                         | `.mtpx` 包及其解压后内容最大字节数   |
| `MEBTTY_PLUGIN_INSTALL_ENABLED`      | `true`                             | 是否允许通过 API/UI 安装第三方插件   |
| `MEBTTY_PLUGIN_SIGNATURE_REQUIRED`   | `false`                            | 预留：是否强制插件签名校验           |
| `MEBTTY_PLUGIN_BACKEND_CODE_ENABLED` | `false`                            | 预留：是否允许可信后端插件代码       |

作为 systemd 服务安装时，Web 应用以 root 启动。创建终端时，用户需要输入本地用户名和密码。PAM 认证通过后，终端进程会降权到该本地用户；如果未填写工作目录，则默认进入该用户的家目录。文件浏览器默认使用活动终端 `cwd` 或最近登录过的本地终端用户家目录，而不是 systemd 服务账户的家目录。
...
│   │   ├── plugins/             # 插件 manifest、安装器、注册表和 API
│   │   │   ├── builtin.py       #   内置插件注册
│   │   │   ├── installer.py     #   .mtpx 上传校验和安装
│   │   │   ├── manager.py       #   插件生命周期工具
│   │   │   └── router.py        #   插件 API 和资源服务
│   │   ├── settings/            # 用户设置模块
│   │   │   └── router.py        #   获取/更新用户偏好
│   │   └── audit/               # 审计日志模块
│   │       ├── router.py        #   审计查询端点
...
| 方法   | 端点                                      | 说明                               |
| ------ | ----------------------------------------- | ---------------------------------- |
| GET    | `/api/plugins`                            | 列出内置和第三方插件               |
| GET    | `/api/plugins/{plugin_id}`                | 获取单个插件                       |
| POST   | `/api/plugins/install`                    | 安装或更新 `.mtpx` 包（仅管理员）  |
| POST   | `/api/plugins/{plugin_id}/enable`         | 启用插件（仅管理员）               |
| POST   | `/api/plugins/{plugin_id}/disable`        | 禁用插件（仅管理员）               |
| DELETE | `/api/plugins/{plugin_id}`                | 删除第三方插件（仅管理员）         |
| GET    | `/api/plugins/{plugin_id}/assets/{path}`  | 获取已启用第三方插件的静态资源     |
...
- `develop` 是日常开发和集成分支。
- `master` 只保留可发布代码和已打 tag 的版本。
- 功能分支应从 `develop` 分出，并合并回 `develop`。
- 发布 tag（`vX.Y.Z` 或 `vX.Y.Z.dev`）必须指向 `master` 可达的提交。
